---
title: "1.8 MySQL索引"
description: "1. 索引是什么 - 索引是对数据库表中一或多个列的值进行排序的结构，是帮助MySQL高效获取数据的数据结构（B+Tree） 2. 索引分类 2.1. 根据是否主键分类 - 主键索引 - 数据列不允许重复，不允许为NULL，一个表只能有一个主键 - 二级索引 - 基本的索引类型，没有唯一性的限制，允"
sourcePath: "Database/MySQL/MySQL索引.md"
category: "database"
...
order: 9
tags: ["Database"]
createdAt: "2021-05-09T06:00:08Z"
updatedAt: "2022-08-26T07:13:21Z"
status: "historical"
language: "zh"
featured: false
indexable: true
...
```sql
SELECT * FROM person_info WHERE name = 'Ashburn' AND birthday = '1990-09-27' AND phone_number = '[已脱敏手机号]';
```

或者
```sql
// 查询优化器会优化这个语句顺序
SELECT * FROM person_info WHERE birthday = '1990-09-27' AND phone_number = '[已脱敏手机号]' AND name = 'Ashburn';
```
...
```sql
// 这个能用到name和birthday列
SELECT * FROM person_info WHERE name = 'Ashburn' AND birthday > '1980-01-01' AND birthday < '2000-12-31' AND phone_number > '[已脱敏手机号]';
```
...
## 8. MySQL索引实现
- [MySQL索引底层实现.md](/notes/database/MySQL/MySQL%E7%B4%A2%E5%BC%95%E5%BA%95%E5%B1%82%E5%AE%9E%E7%8E%B0/)
## 9. 参考

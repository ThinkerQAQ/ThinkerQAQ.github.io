---
title: "2.2 PostgreSQL MVCC"
description: "1. 总体规则 - 在PostgreSQL中，每一个事务都会得到一个被称作为 XID 的事务ID - 可以通过 select cast(txid current() as text) 查询 - 这里说的事务不仅仅是被 BEGIN - COMMIT 包裹的一组语句，还包括单条的 insert 、 up"
sourcePath: "Database/PostgreSQL/PostgreSQL MVCC.md"
category: "database"
...
order: 41
tags: ["Database"]
createdAt: "2021-05-09T04:04:29Z"
updatedAt: "2021-05-09T04:04:51Z"
status: "historical"
language: "zh"
featured: false
indexable: true
...
> 2026 注：这里是简化理解。PostgreSQL 的事务 ID 可以延迟到真正需要时再分配；行可见性也不能只按 XID 大小判断，还要结合快照中的 `xmin` / `xmax` / 活跃事务集合以及事务提交状态。旧的 `txid_*` 查询接口目前主要用于兼容。

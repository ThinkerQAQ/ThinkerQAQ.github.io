---
title: "1.1 Nginx"
description: "Nginx 原始学习笔记：特点、配置、Lua、优化、通用配置与 FAQ。"
...
- 动静分离
Nginx 可以直接处理静态文件，并把动态请求转发给后端应用服务（例如 Tomcat）。
...
数据没准备好，调用直接返回。继续调用
- IO多路复用
数据没准备好时，应用可以阻塞在多路复用等待接口上；一个线程可以同时等待多个文件描述符，而不是为每个连接分配一个线程。
- 信号驱动
数据准备好了，由操作系统负责通知应用
...
I/O 多路复用可以让一个事件循环管理多个连接，每个连接对应的文件描述符注册到 epoll 等事件接口上，
...
```nginx
server {
    # 访问localhost/admin.html会在$HOME/software/code下找admin.html
    location ~ ^/admin.html {
...
- 生成密码：`htpasswd -c ./auth_password nginxuser`

```nginx
location ~ ^/admin.html {
    root   $HOME/software/code;
    auth_basic "Auth access test! Input your password!";
    auth_basic_user_file $HOME/software/nginx/nginx/conf/conf.d/auth_password;
}
...
location ~ .*\.(jpg|gif|png)$ {
    root $HOME/software/code/images;
...
server {
    ...
    location / {
       root $HOME/software/nginx/nginx/html;
       index 8080.html;
    }
}
...
location / {
    ...
    # 最后会访问$HOME/software/code/courses/11/22/course_1.html
...
#### 4.9.1. 语法
现代 Nginx 使用 `listen 443 ssl;` 开启 HTTPS；旧的 `ssl on` 指令已经移除。
...
假设都访问http://localhost/static/imgs/test.png，root会访问$HOME/static/imgs/static/imgs/test.png，alias会访问$HOME/static/imgs/test.png



### 8.4. 获取用户真实的IP

如果 Nginx 位于可信反向代理之后，应先配置可信代理范围，再使用 Real IP 模块解析客户端地址；不能直接信任来自任意客户端的 `X-Forwarded-For`。
...
- try_files 会到硬盘里尝试找这个文件。如果存在名为 $HOME/software/nginx/nginx/cache/example的文件，就直接把这个文件的内容发送给用户。 
- 显然，目录中没有叫 example 的文件。然后就看 $uri/，增加了一个 /，也就是看有没有名为 $HOME/software/nginx/nginx/cache/example/ 的目录。 
- 又找不到，就会 fall back 到 try_files 的最后一个选项 @java_page，根据@java_page配置进行内部重定向。

```nginx

location / {
    root $HOME/software/nginx/nginx/cache;

---
title: "2.3 Redis数据结构"
description: "1. Redis DB redisDb是Redis中表示Db的数据结构，里面包含了dict类型； dict是Redis中表示K、V的数据结构，里面包含了dictht类型； dictht是数组，数组中的每个元素是dictEntry；（即Redis用hash结构实现K、V） dictEntry的next"
sourcePath: "Redis/使用/Redis数据结构.md"
...
updatedAt: "2022-08-26T13:57:19Z"
status: "historical"
language: "zh"
featured: false
indexable: true
...
### 2.1. 使用场景
- 缓存 [如何设计缓存系统.md](/notes/system-design/%E6%8A%80%E6%9C%AF%E7%BB%84%E4%BB%B6/%E5%A6%82%E4%BD%95%E8%AE%BE%E8%AE%A1%E7%BC%93%E5%AD%98%E7%B3%BB%E7%BB%9F/)
- 分布式锁 [Redis分布式锁.md](/notes/redis-cache/%E4%BD%BF%E7%94%A8/Redis%E5%88%86%E5%B8%83%E5%BC%8F%E9%94%81/)
- 计数器
- Web集群session
- 分布式系统全局序列号
...
### 3.1. 使用场景
- 异步队列 [Redis异步队列.md](/notes/redis-cache/%E4%BD%BF%E7%94%A8/Redis%E5%BC%82%E6%AD%A5%E9%98%9F%E5%88%97/)
- 微薄的粉丝列表
### 3.2. 底层实现
- 当数据量比较小的时候，使用ziplist实现：ziplist.md（关联笔记尚未公开）
    - 列表中保存的单个数据（有可能是字符串类型的）小于64字节；
    - 列表中数据个数少于512个
- 当数据量比较大的时候，使用双向循环链表实现：linkedlist.md（关联笔记尚未公开）
## 4. set
...
- 当满足下面这样两个条件的时候， 使用有序数组实现：array.md（关联笔记尚未公开）
    - 存储的数据都是整数；
    - 存储的数据元素个数不超过512个。
- 否则使用hashmap实现：hashmap.md（关联笔记尚未公开）
...
- 当数据量比较小的时候，使用ziplist实现：ziplist.md（关联笔记尚未公开）
    - 元素数量少于128的时候
    - 每个元素的长度小于64字节
- 当数据量比较大的时候使用skiplist实现：跳表.md（关联笔记尚未公开）
...
- hmset细粒度缓存
- hmset+lua脚本实现令牌桶限流算法[Redis RateLimiter.md](/notes/redis-cache/%E4%BD%BF%E7%94%A8/Redis%20RateLimiter/)
...
- 当数据量比较小的时候，使用ziplist实现：ziplist.md（关联笔记尚未公开）
    ...
    - 字典中键值对的个数要小于512个
- 当数据量比较小的时候，使用hashmap实现：hashmap.md（关联笔记尚未公开）

---
title: "1.3 Redis持久化"
description: "1. 为什么要持久化 Redis数据存放在内存，一旦宕机那么内存中的数据全部丢失了。 因此需要把内存中的数据持久化到硬盘中以便挂了之后恢复 2. 持久化方式 2.1. RDB 2.1.1. 是什么 把当前内存中的数据做成一个二进制文件dump到磁盘中，就是一个快照 2.1.2. 触发时机 2.1.2"
sourcePath: "Redis/Redis持久化.md"
...
updatedAt: "2022-05-31T02:53:31Z"
status: "historical"
language: "zh"
featured: false
indexable: true
...
##### 2.2.5.2. 解决
`iotop`或iostat.md（关联笔记尚未公开）观察磁盘负载
#### 2.2.6. 开启AOF
```conf
appendonly  yes

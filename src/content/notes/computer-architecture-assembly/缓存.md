---
title: "2.1 缓存"
description: "CPU Cache、MESI、写缓冲器、无效化队列与内存屏障的历史学习笔记。"
sourcePath: "Computer_Composition_Principle/缓存.md"
...
updatedAt: "2024-03-09T13:22:00Z"
status: "historical"
language: "zh"
featured: false
indexable: true
---
> 2026 注：本文把 MESI、写缓冲器、无效化队列和内存屏障作为简化教学模型来理解。实际缓存一致性协议与内存排序规则依赖具体 CPU 架构；内存屏障也不能简单理解为“清空某个硬件队列”。
...
他把缓存entry 分成四种状态
- Invalid：初始状态。不包含缓存数据
- Shared：每个处理器上的entry可能都有一份一样的缓存。缓存数据与内存数据一致。
- Exclusive：只有一个处理器上的entry保留了一份缓存。缓存数据与内存数据一致。
- Modified：某个处理器更新了缓存行，但并未写入内存。缓存数据与内存数据不一致。

---
title: "5.6 网络调优"
description: "1. 网络性能指标 1.1. 网络层 - 它们主要负责网络包的封装、寻址、路由，以及发送和接收 - 性能指标： - 每秒可处理的网络包数 PPS。 - 可以用内核自带的发包工具 pktgen 测试 1.2. 传输层 - 它们主要负责网络传输 - 性能指标： - 吞吐量（BPS） - 连接数以及延迟 "
sourcePath: "Operating_System/Linux/性能调优/网络调优.md"
...
createdAt: "2021-09-26T07:46:07Z"
updatedAt: "2022-08-29T09:19:57Z"
status: "historical"
language: "zh"
featured: false
indexable: true
...
看了下 game_center_svr 的 ReceiveGameGift 接口（领取礼包）的超时主要是因为 giftpack 服务 Collect 接口慢图片未迁移。
可能一方面需要调用 ams 的发货，另一方面 sql 比较复杂图片未迁移；
再加上提供的是单个领取的不支持批量领取，而我这边是并发调用，量一大就比较容易超时了图片未迁移可以用这个 case 看下：#

## 6. 参考

- [记一次网络丢包导致的访问 mysql 数据库问题 \- KM 平台](#)
- [业务丢包分析实战之微突发 \- KM 平台](#)
- [记一次 golang 服务超时问题定位 \- KM 平台](#)

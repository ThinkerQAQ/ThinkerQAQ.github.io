---
title: "3.4 内存管理"
description: "1. Golang内存分区 1.1. 栈 Golang栈管理.md 1.2. 堆 Golang堆管理.md 1.3. 全局区-静态全局变量区 - 全局变量+常量 - 分配：进程启动时 - 回收：进程退出时，由操作系统回收 1.4. 代码区 2. Golang runtime.MemStat Top "
...
> 2026 注：这里的对应关系不准确。`MemStats.Sys` 不能直接当作 RSS，`Sys` 也不只包含 `HeapSys + StackSys`；RSS 还可能包含 cgo、mmap 等 Go runtime 之外的内存。

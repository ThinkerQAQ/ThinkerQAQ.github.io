---
title: "3.2 逃逸分析"
description: "1. 逃逸分析 如果是Java或C++创建出来的对象一般都是分配在堆上的，而Golang new出来的对象会根据逃逸分析决定是分配在堆还是栈上 2. 为什么需要逃逸分析 堆 栈 --- ----------- ------------- 回收 需要垃圾回收 退出函数即销毁 空间 大 小 3. 如何逃"
...
> 2026 注：这两条适合作为当时的简化理解，不是编译器的绝对规则。实际是否逃逸以当前 Go 编译器的分析结果为准。

...
> 2026 注：这里记录的是旧版 runtime 的实现理解。当前 `mcache` 是 per-P，不是 per-thread；下面固定的 arena / bitmap 大小也属于旧实现细节。
...
## 6. 内存分配流程
根据[逃逸分析.md](/notes/go/%E9%80%83%E9%80%B8%E5%88%86%E6%9E%90/)的结果能在栈上分配就在栈上，否则考虑堆

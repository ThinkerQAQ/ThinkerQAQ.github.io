...
test("只还原当前文章的新近关联元数据，不保存错误和额外字段", () => {
  ...
  state.saveMatches(saved, "example", {
    cnblogs: { text: "远端找到 1 篇候选。", bindings: [{ account: "private" }], items: [{ id: "42", title: "Example", published: true, bound: true, category: "后端", tags: ["Java", 7], token: "never-store" }] },
  ...
  assert.equal(current.matches.cnblogs.items[0].category, "后端");
  assert.deepEqual(Array.from(current.matches.cnblogs.items[0].tags), ["Java"]);

...
import { MERMAID_CLI_PACKAGE } from "./compiler.mjs";
...
export function defaultRun(command, args, { cwd = process.cwd() } = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", (code, signal) => {
      ...
      if (signal) {
        reject(new Error(command + " terminated by signal " + signal + ": " + combined.trim()));
      } else if (code !== 0) {
        reject(new Error(command + " exited with code " + code + ": " + combined.trim()));
      } else {
